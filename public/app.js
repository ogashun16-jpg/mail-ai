...
const state = {
  ...
  composeLang: 'ja',
  replyTo: null,
  composeAttachments: [],  // 作成中の添付ファイル {filename, contentType, size, dataBase64}
  isSyncing: false
};
...
function renderMessageDetail(detail) {
        ...
        <div class="attachments-section">
          ...
          <div class="attachments-grid">
            ${detail.attachments.map((a, idx) => {
              ...
              return `
                <div class="attachment-card" data-uid="${detail.uid}" data-idx="${idx}" data-content-type="${escapeHtml(a.contentType || '')}" data-filename="${escapeHtml(a.filename || '')}" data-can-preview="${canPreview}">
                  ...
                  <div class="attachment-actions">
                    ${canPreview ? `<button class="attach-action-btn preview-btn" title="プレビュー" aria-label="プレビュー">
                      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                    </button>` : ''}
                    <button class="attach-action-btn download-btn" title="ダウンロード" aria-label="ダウンロード">
  ...
  // 添付ファイルのイベントリスナーを設定（onclick文字列の引用符エスケープを回避）
  setTimeout(() => {
    document.querySelectorAll('.attachment-card').forEach(card => {
      ...
      if (previewBtn) {
        previewBtn.addEventListener('click', (e) => {
          e.stopPropagation();
          previewAttachment(uid, idx, contentType, filename);
        });
      }
      ...
      if (downloadBtn) {
        downloadBtn.addEventListener('click', (e) => {
          e.stopPropagation();
          downloadAttachment(uid, idx, filename);
        });
      }

      // カード全体をタップしてもプレビュー（プレビュー可能な場合のみ）
      if (card.dataset.canPreview === 'true') {
        card.addEventListener('click', (e) => {
          if (e.target.closest('.attach-action-btn')) return;
          previewAttachment(uid, idx, contentType, filename);
        });
...
// ===== 添付ファイル =====
// iOS Safari対応: blob経由でダウンロード処理
async function downloadAttachment(uid, idx, filename) {
  ...
  try {
    ...
    // Web Share APIが使えるならネイティブの共有を使う（iOS向けに最適）
    ...
    toast('ダウンロードエラー: ' + err.message, 'error');
  }
}
...
function openCompose() {
  ...
  // ファイル選択もリセット（同じファイルを連続で添付できるように）
...
async function handleFileAttach(event) {
  ...
  for (const file of files) {
    if (file.size > MAX_SIZE) {
      toast(`「${file.name}」は10MBを超えるため添付できません`, 'error');
      continue;
    }
    ...
    try {
      ...
      toast(`「${file.name}」の読み込みエラー: ${err.message}`, 'error');
    }
  }
...
function fileToBase64(file) {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      // data:image/png;base64,XXXX の形式から、base64部分のみ取り出す
...
function renderComposeAttachments() {
  ...
  count.textContent = state.composeAttachments.length;

  list.innerHTML = state.composeAttachments.map((a, idx) => {
    const ext = (a.filename || '').split('.').pop().toUpperCase().substring(0, 4);
    const sizeKB = Math.round(a.size / 1024);
    const sizeDisplay = sizeKB > 1024 ? `${(sizeKB / 1024).toFixed(1)} MB` : `${sizeKB} KB`;
    return `
      <div class="compose-attach-item">
        <div class="compose-attach-item-icon">${ext || '📄'}</div>
        <div class="compose-attach-item-name" title="${escapeHtml(a.filename)}">${escapeHtml(a.filename)}</div>
        <div class="compose-attach-item-size">${sizeDisplay}</div>
        <button class="compose-attach-item-remove" onclick="removeAttachment(${idx})" title="削除">✕</button>
      </div>
    `;
  }).join('');
}
...
async function sendMail() {
  ...
  // 添付ファイルの合計サイズチェック（25MB制限）
  const totalSize = state.composeAttachments.reduce((sum, a) => sum + a.size, 0);
  if (totalSize > 25 * 1024 * 1024) {
    toast('添付ファイルの合計が25MBを超えています', 'error');
    btn.disabled = false;
  ...
  try {
    ...
    const attachInfo = state.composeAttachments.length > 0 ? `（添付 ${state.composeAttachments.length} 件）` : '';
    toast(`✓ 送信完了${attachInfo}`);
    document.getElementById('composeModal').classList.remove('open');
    document.getElementById('composeBody').value = '';
    state.replyTo = null;
    state.composeAttachments = [];
