...
async function loadFolder(path, type) {
  ...
  // モバイルでフォルダ選択時はリストペインを表示
  if (window.innerWidth <= 820) showPane('listPane');
...
async function openMessage(uid) {
  ...
  if (window.innerWidth <= 820) showPane('detailPane');
  ...
  document.getElementById('detailContent').innerHTML = `
    <div class="detail-actions">
      <div class="action-group">
        <button class="action-btn back-btn" onclick="showPane('listPane')">
