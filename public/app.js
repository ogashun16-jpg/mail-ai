...
const html = standardFolders.map(f => `<div class="nav-item ${f.path === state.currentFolderPath ? 'active' : ''}" onclick="loadFolder('${escapeHtml(f.path)}', '${f.type}')"> <span class="nav-icon">${f.icon}</span> <span class="nav-label">${f.label}</span> </div>`).join(’’) + (custom.length > 0 ? `<div class="section-label">その他</div> ${custom.map(f =>`
...
document.getElementById(‘detailContent’).innerHTML = `<div class="detail-actions"> <div class="action-group"> <button class="action-btn back-btn" onclick="showPane('listPane')"> <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.6"><path d="M10 4 6 8l4 4"/></svg> </button> <button class="action-btn" onclick="replyMessage()"> <svg width="13" height="13" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"><path d="M9 4 5 8l4 4M5 8h7"/></svg> 返信 </button> <button class="action-btn" onclick="forwardMessage()"> <svg width="13" height="13" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"><path d="M7 4l4 4-4 4M11 8H4"/></svg> 転送 </button> </div> <div class="action-group"> ${isSpam ? '' :`<button class="action-btn" onclick="moveToSpam()" title="迷惑メールへ">🚫</button>`}
...
<div class="detail-scroll">
  ...
  ${detail.attachments && detail.attachments.length > 0 ? `
    <div style="margin-top:24px;padding:14px;background:var(--bg-list);border-radius:10px;">
      <div style="font-size:11px;font-weight:600;color:var(--ink-3);margin-bottom:8px;letter-spacing:0.05em;">添付ファイル (${detail.attachments.length})</div>
      ${detail.attachments.map(a => `
        <div style="font-size:12px;padding:4px 0;">📎 ${escapeHtml(a.filename)} <span style="color:var(--ink-3);">(${Math.round(a.size/1024)}KB)</span></div>
      `).join('')}
    </div>
  ` : ''}
</div>
...
body.innerHTML = `<div class="ai-loading"> <span class="ai-loading-dot"></span> <span class="ai-loading-dot"></span> <span class="ai-loading-dot"></span> <div style="margin-top:12px;">Claude が要約を生成中…</div> </div>`;
...
body.innerHTML = `<div class="ai-error">⚠️ ${escapeHtml(err.message)}</div> <button class="ai-trigger" onclick="generateSummary()" style="margin-top:10px;">再試行</button>`;
}
}
