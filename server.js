...
// 添付ファイルのダウンロード/プレビュー
// ============================================================
app.get('/api/message/:uid/attachment/:idx', requireAuth, async (req, res) => {
  const uid = parseInt(req.params.uid);
  const idx = parseInt(req.params.idx);
  const folder = req.query.folder || 'INBOX';
  const disposition = req.query.disposition || 'inline'; // 'inline' or 'attachment'
  ...
  try {
    ...
    try {
      ...
      res.setHeader(
        'Content-Disposition',
        `${dispositionType}; filename="${filename.replace(/"/g, '_')}"; filename*=UTF-8''${encodedFilename}`
      );
...
app.post('/api/send', requireAuth, async (req, res) => {
  ...
  try {
    // 添付ファイルをnodemailer形式に変換
    // クライアントから送信される形式: { filename, contentType, dataBase64 }
